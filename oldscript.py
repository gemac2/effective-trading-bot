...
def get_klines_fifteen_minutes(tick):
    try:
        klines = client.futures_klines(symbol=tick, interval=Client.KLINE_INTERVAL_15MINUTE, limit=48, timeout=30)
        timeframe = "15 Minutes"
    except Exception as e:
        print(f"Error while getting data for {tick} 15 minutes klines: {e}")
        return None, None
...
def main_loop():
    while True:
        ...
        for tick in ticks:
            ...
            klines_15m, time15m = get_klines_five_minutes(tick)
            if klines_15m is not None and time15m is not None:
                found_signal_bollinger = get_bollinger_signals(tick, klines_15m, time15m)
                if found_signal_bollinger:
                    print("Found signal for", tick, "on 15 minutes timeframe")
                    print('**************************************************')
                    print('')
