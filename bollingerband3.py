...
def search_ticks():
    ...
    for tick in list_ticks:
        if tick['symbol'].endswith('USDT'):
            ...
            if tick['symbol'] not in ("USDCUSDT", "BTCUSDT"):
...
def place_futures_order(symbol, side, entry_price):
    ...
    if quantity <= 0:
        print("Cantidad calculada <= 0. No abrimos operación.")
    ...
    if side == "LONG":
        ...
        try:
            ...
            print(f"[{symbol}] Orden MARKET BUY (LONG) ejecutada. Qty={quantity}")
        except Exception as e:
        ...
        try:
            ...
            print(f"[{symbol}] Orden MARKET SELL (SHORT) ejecutada. Qty={quantity}")
        except Exception as e:
...
def apply_strategy_bollinger_rsi_5m(symbol, klines):
    ...
    if (min_low < lower_band.iloc[-1]) and (close_price <= lower_band.iloc[-1]) and (rsi < 30):
        # Verificamos volumen
        ...
        if volume >= IDEAL_VOLUME:
            ...
            if symbol in open_positions:
                print(f"Ya existe {symbol} en open_positions. No abrimos otra.")
                return False

            entry_price = float(info['lastPrice'])
            stop_id, tp_id, qty = place_futures_order(symbol, side="LONG", entry_price=entry_price)
    ...
    # Condiciones de LONG
    if (max_high > upper_band.iloc[-1]) and (close_price >= upper_band.iloc[-1]) and (rsi > 70):
        info = get_info_tick(symbol)
        if not info:
            return False
        ...
        if volume >= IDEAL_VOLUME:
            ...
            entry_price = float(info['lastPrice'])
            stop_id, tp_id, qty = place_futures_order(symbol, side="SHORT", entry_price=entry_price)
            if stop_id and tp_id:
                open_positions[symbol] = {
                    "side": "SHORT",
                    "position_size": qty,
